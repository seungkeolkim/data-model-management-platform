...
배경:
    pipeline.png(graphviz) 시각화에서 노드 라벨이 너무 길어져 가독성이 떨어진다.
    "_classification" postfix 는 일반 단어의 풀네임 정책에 부합하지만, 다수 노드가
    동시에 표시될 때 라벨 폭이 비대해지는 부작용이 컸다.
    축약어는 가능한 한 지양하나 시각화 가독성을 위해 본 도메인에 한해 "cls_" prefix
    규약을 도입한다. prefix 통일은 IDE 파일 트리에서 같은 도메인 노드끼리 자연 정렬되는
    부수 효과도 있다.

조치:
    UPDATE manipulators SET name = REPLACE(...) — 10종 일괄 rename.
    (코드 측 클래스명/파일명/문자열은 별도 커밋에서 동기화)
"""
