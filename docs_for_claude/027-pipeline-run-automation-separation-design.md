...
### 12-8. 현재 상태 스냅샷 (최신 갱신: 2026-04-24 §9-4 종료)
...
- **§9 진행 상황**:
    ...
    - ✅ §9-3 — **서비스 + 라우터 MVP 완료**. Pipeline CRUD + PipelineAutomationService
      + submit_run. backend 446/446 통과
    - ✅ §9-4 — **schema_version 2 전환 완료**. lib/pipeline/config.py CURRENT_SCHEMA_VERSION=2
      + passthrough_source_split_id 필드 + extract_source_split_ids 헬퍼(§12-9).
      backend submit_run / automation rerun 이 v1/v2 분기. DatasetSummary.split_id 노출.
      FE DataLoad 노드 버전 Select 제거 → `(group, split)` 2단계만. saveDefinition /
      graphToConfig / configToGraph 도 v2 대응. backend 446/446 + FE tsc pre-existing
      11건 외 신규 0
    - ⬜ §9-5 ~ §9-9 — 미착수
...
- **재개 시 체크리스트 (§9-5 이어서)**:
    1. §9-5 Celery task — `pipeline_tasks.run_pipeline` 이 PipelineRun 으로부터
       `pipeline_id` / `automation_id` / `transform_config` 를 읽어 기록하는지 확인.
       현재는 `run_pipeline.delay(run_id, pipeline_config)` 인데 dispatch 시점에 이미
       PipelineRun 행이 만들어졌으므로 큰 수정 없음. schema_version=2 config 도 동일 경로
    2. §9-6 Version Resolver Modal — Pipeline 목록 행 "실행" 버튼 → 모달에서
       `resolved_input_versions = {split_id: version}` 받고 `POST /pipelines/entities/{id}/runs`
    3. §9-7 `/pipelines` 페이지 재배선 — `/pipelines` Pipeline 목록, `/pipelines/:id`
       상세, `/pipelines/runs` 전체 실행 이력. 사이드바 메뉴 2개 분리
    4. §9-8 Automation 메뉴 실 API 연결 — 026 목업 fixture → 실 엔드포인트 교체.
       §12-6 reassign 노티. legacy Pipeline 은 실행/자동화 버튼 비활성
    5. §9-9 legacy 필터 + 최종 회귀 + 핸드오프 028 작성 + 설계서 v7.10 승격
- **§9-4 에서 남긴 후속 정리 (지금은 정상 동작, 기록만)**:
    - v1 legacy config 상세 진입 시 엣지 복원 미완 (DataLoad 노드가 v2 만 수거). is_active=FALSE
      라 실행 불가이므로 기능상 문제 없음. §9-7 에서 "legacy" 배지 + 설명 Alert 추가 권장
    - FE `DatasetSummary.pipeline_execution_id` 필드는 이제 API 가 `pipeline_run_id` 로
      내려주므로 undefined. UI 영향 없지만 타입 이름 일치는 §9-7 에서 정리
