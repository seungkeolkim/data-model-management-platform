...
### 12-8. 현재 상태 스냅샷 (최신 갱신: 2026-04-24 §9-3 MVP 종료)
...
- **§9 진행 상황**:
    ...
    - ✅ §9-2 — **ORM rename 완료**. Pipeline / PipelineAutomation ORM 신규 +
      PipelineExecution → PipelineRun rename + 호출부 10개 파일 전수 교체
    - ✅ §9-3 — **서비스 + 라우터 MVP 완료**. Pipeline CRUD (list/get/patch) +
      PipelineAutomationService (upsert/soft-delete/reassign/rerun) + submit_run +
      Pydantic 스키마. 주요 API 엔드포인트 전부 스모크 통과. backend 446/446 통과
    - ⬜ §9-4 ~ §9-9 — 미착수
...
- **재개 시 체크리스트 (§9-4 이어서)**:
    1. FE `frontend/src/pipeline-sdk/definitions/dataLoadDefinition.tsx` 에서
       `datasetId` / `version` 필드 제거. `(group, split)` 까지만 받도록
    2. `frontend/src/pipeline-sdk/engine/graphToConfig.ts` — `source:<split_id>` 생성
    3. `frontend/src/pipeline-sdk/engine/configToGraph.ts` — schema_version 1 은
       placeholder 로 복원 (읽기 전용)
    4. `lib/pipeline/config.py` 에 `CURRENT_SCHEMA_VERSION = 2` 상수 + `extract_source_split_ids`
       순수 헬퍼 추가 (§12-9)
    5. backend `_validate_with_database` → `validate_structural` / `validate_runtime`
       분리 (§12-5), schema_version=2 분기 추가
    6. (선택) `POST /pipelines/entities` 엔드포인트 신설 → `submit_pipeline` 의 임시
       `_get_or_create_pipeline_for_submit` 제거
