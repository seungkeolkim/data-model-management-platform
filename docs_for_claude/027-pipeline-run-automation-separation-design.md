...
### 12-8. 현재 상태 스냅샷 (최신 갱신: 2026-04-24 §9-7 종료, UI 스모크 대기)

- **브랜치** — `feature/pipeline-run-automation-separation`
- **머지 대상** — 현행 `main = 985acc2`
- **§9 진행 상황**:
    - ✅ §9-1 Alembic 031 (57 legacy 백필, 왕복 성공)
    ...
    - ✅ §9-5 Celery executor v2 치환 — `_substitute_v2_to_resolved_config` 순수 헬퍼가
    ...
    - ✅ §9-7 `/pipelines` 재배선 — PipelineListPage (신규), `/pipelines/runs` (기존 이력).
      사이드바 "데이터 변형" 2 하위 (목록 / 실행 이력)
    - ⬜ §9-8 Automation 메뉴 실 API 연결 — 다음 세션
    - ⬜ §9-9 UI 스모크 피드백 + 최종 회귀 + 핸드오프 028 — 다음 세션
