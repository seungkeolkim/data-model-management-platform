...
> **작업지시서 v7.2** | Classification DAG 에디터 + Celery runner 완성 + manipulator prefix 통일(det_/cls_) 반영 baseline
> 기준일: 2026-04-15 (post-session)
> 이전 설계서: `docs_history/objective_n_plan_6th.md`
> 통합 핸드오프: `docs_for_claude/017-classification-dag-ready-handoff.md` (이전: 016은 `docs_history/handoffs/`)
...
### 2-4. MANIPULATOR_REGISTRY — 자동 발견 (22종: det 12 + cls 10)
...
**Classification 10종 (`cls_` prefix, 전부 stub — 실구현은 다음 세션):**
- 바이너리 불변 / schema·manifest 조작: `cls_rename_head`, `cls_rename_class`, `cls_reorder_heads`, `cls_reorder_classes`, `cls_select_heads`
- 레코드 필터: `cls_filter_by_class`, `cls_remove_images_without_label`, `cls_sample_n_images`
- 병합: `cls_merge_classes` (head 내 class 통합), `cls_merge_datasets` (`accepts_multi_input=True`)
...
### 2-11. Classification merge 정책 (설계 확정 · 2026-04-16, 구현 대기)
...
- Multi-label head 는 class 별로 3-값 상태를 판정:
  ...
  - `unknown`: 해당 측 `classes` 에 class 가 **애초에 없었음** (판단 안 함)
...
4. **Classification operator 10종 실구현** [1순위 — 다음 세션 주제]
   ...
   - 3차: 머지 (`cls_merge_classes`, `cls_merge_datasets` — 정책은 **§2-11** 에 확정됨)
   - 선행: `lib/pipeline/io/` CLS_MANIFEST 쓰기 경로를 operator 결과 기준으로 재검증, 정합성 audit 유틸 도입
5. **Automation 실구현** — 정책 확정 대기 (lineage 조정 + minor 버전 증가 규약)
6. **미구현 Detection manipulator 2종** — `det_change_compression` / `det_shuffle_image_ids`
7. **버전 정책 운영 검증** — automation과 함께
8. **Phase 3** — TrainingExecutor/GPUResourceManager 인터페이스, 알림 골격, GNB/Manipulator/시스템 상태 페이지, 전체 UX 정리
9. **Step 2** 진입 — DockerTrainingExecutor, nvidia-smi 기반 GPUResourceManager, MLflow, Prometheus+DCGM, SMTP 알림
10. **Step 3 이후** — S3StorageClient, KubernetesTrainingExecutor, Helm, Argo/Kubeflow, Volcano, KEDA, MinIO
11. **Step 4** — Label Studio, Synthetic Data, Auto Labeling, Offline Testing, Auto Deploy, 데이터 자동 수집
12. **Step 5** — Generative Model MLOps
