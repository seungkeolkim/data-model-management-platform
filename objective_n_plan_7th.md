...
**현재 12종:**
- `det_format_convert_to_coco` / `det_format_convert_to_yolo` / `det_format_convert_from_coco` / `det_format_convert_from_yolo` (no-op — 통일포맷 도입 후 의미만 유지)
- `det_merge_datasets`
...
5. **미구현 manipulator 2종** — `det_change_compression` / `det_shuffle_image_ids` (detection용)
