...
> - `db7ac68` chore(migration): Classification manipulator 파라미터 label 6건 축약 (Alembic 028)
...
### 1-10. Classification manipulator 파라미터 label 6건 축약 (Alembic 028, 커밋 `db7ac68`)

**배경.** DAG 박스 안에서는 `schema.label` 이 그대로 `OperatorNodeComponent` 에 표시되어,
label 이 길수록 노드 가로폭이 비정상적으로 늘어나 DAG 전체 가독성이 떨어졌다. 속성 패널에서도
panel 폭이 좁은 경우 `Form.Item` 라벨이 오른쪽에서 잘려 보이는 문제 발견.

**결정.** `DynamicParamForm` 에 tooltip / help 필드가 아직 없고, 추가 필드 도입은 규모가 크므로,
**label 자체를 짧게 줄이고 상세 의미는 설계서 / manipulator docstring 으로 위임**. Alembic 028 로
일괄 치환.

**대상 6건 (`jsonb_set` 으로 label 필드만 타겟 — 다른 스키마 속성은 건드리지 않음).**

| manipulator | param | before | after |
|---|---|---|---|
| `cls_add_head` | `class_candidates` | Class 후보 (줄바꿈 구분, 2개 이상, 순서 = 학습 output index) | Class 후보 (줄바꿈 구분, 2개 이상) |
| `cls_add_head` | `multi_label` | Multi-label 여부 (체크 시 한 이미지가 여러 class 를 동시에 가질 수 있음) | Multi-Label 여부 |
| `cls_set_head_labels_for_all_images` | `set_unknown` | Unknown 으로 초기화 (체크 시 classes 무시하고 모든 라벨을 null 로) | Unknown 으로 초기화 |
| `cls_set_head_labels_for_all_images` | `classes` | 설정할 Class 이름 (줄바꿈 구분, set_unknown 미체크 시 사용). single-label 은 정확히 1개, multi-label 은 0개 이상(빈 값 허용). | 설정할 Class 이름 (줄바꿈 구분) |
| `cls_filter_by_class` | `include_unknown` | Unknown(null) 라벨도 매칭 대상에 포함 (체크 시 labels[head] 가 null 인 이미지도 match) | Unknown 라벨 포함 |
| `cls_filter_by_class` | `classes` | 대상 Class 이름 (줄바꿈 구분, any match). 비워두면 include_unknown 조합으로만 판정. | 대상 Class 이름 (줄바꿈 구분) |

**향후 확장 여지.** tooltip / help 필드를 `params_schema` 에 정식 추가해 DynamicParamForm 이
`<Tooltip title=help>` 로 렌더링하도록 하면 label 은 짧게 유지하면서도 상세 설명을 되살릴 수 있다.
현재 handoff 범위 밖이므로 장기 UX TODO 로만 기록.
...
- 설계서: `objective_n_plan_7th.md` §2-4 (registry 26종), §2-13 (filename-identity), §5 (남은 작업), §6-1 (이미지 변형 rename 규약 신설), §6-2 (binary label type 결정)
- 이전 핸드오프: `docs_history/handoffs/020-classification-filename-identity-handoff.md`
...
본 문서는 `feature/classification-dag-implementation-01` 브랜치의 마지막 실구현 챕터 (cls 이미지
변형 2종 + head 조작 2종 + 이미지 필터 1종 + label 축약) 로 종결된다. 브랜치 전체 (017 ~ 021)
의 정리 및 main 머지 직전 요약은 후속 핸드오프 **`docs_for_claude/022-classification-dag-chapter-closure-handoff.md`**
에 정리된다.
