...
- `cls_merge_datasets` — ✅ 실구현 완료 (`objective_n_plan_7th.md §2-11`).
  `accepts_multi_input=True`. params 3종(`on_head_mismatch` / `on_class_set_mismatch` /
  `on_label_conflict`) 로 head·class 불일치와 라벨 충돌 처리 방침을 사용자가 선택한다.
  호환성 검증은 `lib/pipeline/cls_merge_compat.py` 에 단일 소스로 추출되어 manipulator
  런타임 (ValueError 방어) 과 `PipelineService._validate_with_database` (FE 노드 이슈
  분배) 양쪽에서 동일 규칙을 공유한다. FE `mergeDefinition.tsx` PropertiesComponent 에서
  DynamicParamForm + `multi_label_union` 승격 확인 모달 제공. 동작 요약 로그는
  processing.log 에 rename/drop/promote 이벤트 + 요약 한 줄로 남는다.
...
8. `cls_merge_datasets` 착수 시 `objective_n_plan_7th.md §2-11` (merge 정책 확정본) 먼저 통독 — 옵션 3종, 정적/런타임 검증 분리, 상충 판정 규칙이 명시되어 있다
